...
import com.za.zenith.world.TreecapitatorService;
...
public class LogBlockDefinition extends BlockDefinition {
    ...
    public boolean onUse(World world, BlockPos pos, Player player, ItemStack heldStack, float hitX, float hitY, float hitZ) {
        ...
        java.util.List<com.za.zenith.world.recipes.IRecipe> recipes = com.za.zenith.world.recipes.RecipeRegistry.getRecipesByType("carving");
        
        if (recipes.isEmpty()) {
            com.za.zenith.utils.Logger.info("No carving recipes found in registry");
        }

        for (com.za.zenith.world.recipes.IRecipe r : recipes) {
            com.za.zenith.world.recipes.CarvingRecipe recipe = (com.za.zenith.world.recipes.CarvingRecipe) r;
            ...
            if (blockMatch) {
                com.za.zenith.utils.Logger.info("Carving debug: toolMatch=%b (held:%s, req:%s), sneaking=%b, hitY=%.2f", 
    ...
    public boolean onBlockBreak(World world, BlockPos pos, Block block, Player player) {
        ...
        int logsAbove = com.za.zenith.world.TreecapitatorService.getInstance().countLogsAbove(world, pos);
        if (logsAbove == 0) {
            return true;
        }

        // Всегда начинаем с первой стадии (zenith:felling_stage_1)
        Identifier stage1Id = Identifier.of("zenith:felling_stage_1");
        ...
        if (stageDef != null) {
            ...
            com.za.zenith.utils.Logger.info("Log started felling: converted to %s, woodIndex: %d", stage1Id, woodIndex);
            return false;
        }
