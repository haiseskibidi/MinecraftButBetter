...
- **Particle.java**: Базовый абстрактный класс для всех частиц в мире. Хранит позицию, скорость, вращение, время жизни и логику удаления.
- **ShardParticle.java**: Реализация процедурного осколка. Содержит данные о материале (`WOOD`, `LEAVES`, `GENERIC`), сид для генерации формы, индекс текстуры и флаг тинтовки. Реализует специфическую физику (например, планирование для листвы).
- **ParticleManager.java**: Глобальный синглтон для управления жизненным циклом частиц. Отвечает за спавн осколков при разрушении блоков (`spawnShatter`), проверку коллизий с хитбоксами блоков и гибридную логику (разные текстуры для слоев травы/земли).

### com.za.zenith.engine.graphics
- **ParticleRenderer.java**: Высокопроизводительный рендерер частиц. Использует инстансинг для отрисовки процедурных треугольников. Реализует логику Billboarding (взгляд на камеру) и передает параметры времени (`uTime`) для шейдерных анимаций.
