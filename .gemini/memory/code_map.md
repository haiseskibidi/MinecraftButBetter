...
### com.za.minecraft.world.blocks.FellingLogBlockDefinition (NEW)
Назначение: Универсальная логика стадий срубания дерева.
Функции: Обрабатывает прогресс разрушения (стадии 1-4). На финальной стадии запускает BFS-поиск всех связанных блоков дерева выше и вокруг, консолидирует дроп и очищает мир.

### com.za.minecraft.world.blocks.WoodTypeRegistry (NEW)
Назначение: Реестр для маппинга типов древесины.
Функции: Сопоставляет числовые индексы (0-5) с `Identifier` оригинальных бревен. Используется для хранения типа дерева в метаданных универсальных стадий.
...
Логика: 
- Динамически подставляет `stripped_` текстуры для стадий срубания.
