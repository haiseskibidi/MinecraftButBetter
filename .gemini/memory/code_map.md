...
Назначение: Основной цикл рендеринга и управление GPU-ресурсами.
Функции: Управляет асинхронной генерацией мешей через `ExecutorService`. Хранит `proxyMeshCache` для поврежденных блоков. Реализует L1 Cache для освещения сущностей.
...
Назначение: Расчет распространения света.
Функции: Использует `LightNode pooling` и `ArrayDeque` для минимизации работы GC при BFS.

### com.za.zenith.entities.Inventory (UPDATED)
