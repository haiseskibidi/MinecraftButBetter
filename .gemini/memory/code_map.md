...
- **Mesh.java**: Обертка над GPU-ресурсами. Поддерживает **Vertex Compression** (28 байт), классический формат и **Mesh Pooling**. Инкапсулирует `baseVertex`, `firstIndex` и `poolVersion` для корректной работы в батчах.
- **MeshPool.java (NEW)**: Глобальный аллокатор GPU-памяти. Владеет гигантскими VBO/EBO (1.5 ГБ), исключая переключение буферов. Реализует систему **Versioning** для безопасного Wrap-around.
- **MultiDrawBatch.java (NEW)**: Оркестратор групповой отрисовки. Управляет собственным VAO, заполняет `IndirectBuffer` и `InstanceBuffer` для вызова `glMultiDrawElementsIndirect`.
- **Renderer.java (Zenith v1.5)**: Главный контроллер. Реализует **MultiDraw Batching**, сводя отрисовку мира к 2-4 вызовам. Внедрен робастный BFS-обход видимости, работающий в пустоте. Управляет жизненным циклом `MeshPool`.
