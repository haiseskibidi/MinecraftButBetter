...
- **Mesh.java**: Обертка над GPU-ресурсами. Поддерживает **Vertex Compression** (7 float) и стандартный формат (16 float). Реализует метод `render(Shader)`, который автоматически выставляет униформу `uIsCompressed` в зависимости от формата меша, обеспечивая безопасную инкапсуляцию стейта.
- **Renderer.java (Zenith v1.1)**: Главный контроллер отрисовки. Реализует **Draw Call Batching**, радиальную спиральную прогрузку и **Front-to-Back Sorting** для оптимизации Early Z-culling. Управляет всеми проходами рендеринга (мир, шрамы, сущности).
