...
### 🚀 Архитектура и Движок (Zenith v2.6)
...
### 🌍 Мир и Генерация (World Expansion v1.1)
- **New Branch: `feature/natural-caves-v2`**: Ветка для тонкой настройки новой генерации пещер.
- **Surface-Only 3D Caves**: "Пузырчатые" 3D пещеры теперь ограничены высотой (плавное затухание от Y=80 до Y=0). Глубже Y=0 мир теперь гарантированно плотный, что исправляет баг с бесконечными падениями и ломаным спавном.
- **Vibe Hole Tuning**: Глобальный каньон в горах (weirdness-based) ослаблен в 5 раз и поднят выше Y=40, чтобы сохранить визуальный стиль без ущерба для геймплея.
