...
## Реализованные фичи
- **Identifier-First Dynamic ID System (NEW)**:
  - **Auto-Assignment**: Полный переход от ручного управления `id` в JSON к динамической регистрации на основе `Identifier`.
  - **Massive Cleanup**: Из всех 200+ JSON-файлов блоков и предметов удалены поля `id`, что исключает конфликты при моддинге и упрощает разработку.
  - **Shader Integration**: Шейдеры переведены на унифицированное декодирование `BlockInfo` (через флаги в ID), что восстановило работу Connected Textures и Tinting при динамических ID.
- **Advanced Developer Panel & UI Components (NEW)**:
  - **UISearchBar Component**: Создан мощный переиспользуемый компонент поисковой строки с поддержкой курсора, выделения, буфера обмена (Ctrl+C/V) и зажатия клавиш.
  - **Smart Search Engine**: Реализован `ItemSearchEngine` для фильтрации по русским названиям, английским ID и путям.
  - **Logical Sorting**: В дев-панели блоки теперь всегда идут первыми, а весь список отсортирован по алфавиту.
