...
## Реализованные фичи
- **Voxel Lighting System (v1.0 NEW)**:
  - **Dual-Channel Data**: Хранение 8 бит света на блок (4 бита Sunlight, 4 бита Blocklight) в `lightData` чанка.
  - **BFS Propagation Engine**: Реализован алгоритм заливки светом (Breadth-First Search) для распространения блочного света от источников (эмиссия) и первичного солнечного света.
  - **Smooth Lighting**: Динамическая интерполяция уровней освещенности между вершинами блоков, устраняющая резкие границы вокселей.
  - **Screen-Space Ambient Occlusion (Simplified)**: Мягкое затенение углов и ниш на основе проверки соседних непрозрачных блоков при генерации меша.
  - **Shader Integration**: Полная поддержка воксельного света в вершинном и фрагментном шейдерах через новые атрибуты вершин (`vLight`, `vAO`).
- **Celestial & Day/Night System (v1.0 NEW)**:
