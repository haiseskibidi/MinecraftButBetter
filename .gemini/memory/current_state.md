...
## Реализованные фичи
- **Dismantling & Progressive Drops (NEW)**:
  - Реализована механика разбора объектов (например, `rusty_car_chassis`). Предметы (металлолом, проволока) вылетают из блока прямо в процессе ударов по Weak Spots (`drop_on_hit`).
  - Исправлен эксплойт бесконечного фарма: выпадение ресурса наносит блоку значительный урон.
- **World Block Damage & Healing System (NEW)**:
  - **Data Persistence**: Урон блокам и история ударов (шрамы) сохраняются в `World.java` (`blockDamageMap`). Прогресс не сбрасывается при отведении взгляда.
  - **Block Healing & Scar Fading**: Природные блоки постепенно восстанавливают HP со скоростью `healing_speed`. По мере "заживления" шрамы на блоке исчезают один за другим.
