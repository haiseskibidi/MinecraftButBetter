...
- **Tactical Minimap (Radar) v1.0 (NEW)**:
  - **Fast Top-Down Sampling**: Реализован алгоритм быстрого сканирования поверхности мира через прямой доступ к `Chunk.blockData` (без аллокаций объектов).
  - **Dynamic Texture Radar**: Использование `DynamicTexture` (128x128) с обновлением через `glTexSubImage2D`.
  - **Player-Centric Rotation**: Карта и маркеры динамически вращаются вслед за взглядом игрока, обеспечивая тактическую ориентацию.
  - **SDF Masking & Scanlines**: Шейдерная обработка (`minimap_fragment.glsl`) обеспечивает идеальную круглую форму, виньетирование и эффект электронных сканирующих линий.
  - **Entity Tracking**: Визуализация игрока (центр) и окружающих сущностей (красные точки для скаутов, синие для предметов).
  - **Blueprint Integration**: Рамка и компасные деления реализованы через систему `GraphicBlueprint` поверх текстуры мира.
