...
#### 1. Data-Driven Animation System (NEW)
Внедрена универсальная система анимаций на основе JSON-конфигураций (`parkour_animations.json`).
...
- **Animation Tracks**: Атомарные потоки данных для отдельных параметров (Tilt, Roll, FOV).
- **Mirrored Tracks**: Механика автоматического инвертирования значений трека на основе внешнего множителя. Используется для процедурного чередования рук/сторон при движении.
- **Jitter Logic**: Процедурное дрожание камеры, настраиваемое через JSON (окно активности, интенсивность).
