...
2.  **Impact Wobble Animation**:
    *   **Proxy Rendering**: При ударе рендерер создает временный "прокси-меш" блока (`breakingMesh`) в координатах цели.
    ...
    *   **Vertex Deformation**: Шейдер `vertex.glsl` деформирует прокси-меш относительно центра `vec3(0.0, 0.5, 0.0)`, создавая эффект пружинящего блока.
