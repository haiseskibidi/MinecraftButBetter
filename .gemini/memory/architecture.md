...
2.  **Impact Wobble Animation**:
    ...
    *   **Carving Synchronization (NEW)**: Рендерер динамических масок (`CarvingRenderer`) синхронизирован с системой Wobble. Маска пня использует тот же `uIsProxy` и ту же матрицу трансформации, что и основной блок, что исключает визуальное расслоение при деформации.
