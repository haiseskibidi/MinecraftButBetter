...
5. **Двойные растения (Double Plants)**:
...
6.  **Raycast**: Алгоритм выбора блоков и сущностей учитывает каждый `AABB` внутри формы, позволяя взаимодействовать с объектами сквозь их пустые части.
7.  **Коллизии**: Метод `Entity.move` итерируется по всем боксам формы блока, обеспечивая точное перемещение по плитам и лестницам.
8.  **Visual Fallbacks (NEW)**: Система `Texture` автоматически генерирует пурпурно-черную шахматку при сбое загрузки ассета, предотвращая падение игры.
9.  **Shader-Based Masking (UPDATED)**: Универсальная система динамического "вырезания" текстур на основе 16-битной маски (`faceMask`), передаваемой в `fragment.glsl`.
    - Используется для плавного отображения прогресса обтёсывания бревен.
    - **Modular Carving**: `CarvingRenderer` динамически запрашивает текстуры обтёсанного слоя из `BlockRegistry` (поле `top`), что позволяет поддерживать новые породы дерева без правки кода.
    - **Precision Mapping**: Система использует попиксельный расчет `overlayUV` для корректного наложения декалей из общего атласа.
...
## Parkour & Cinematic Camera System (NEW)
...
### 1. Физика паркура (Clamber System)
- **Bezier 3D Path**: В отличие от раздельной интерполяции осей, подъем на уступ реализован через единую квадратичную кривую Безье. Это гарантирует отсутствие рывков при смене векторов скорости.
- **Ledge Detection**: Использует многоступенчатый рейкаст для поиска подходящего края блока в диапазоне высот от 1.25 до 2.45 метров.
- **State Machine**: Управляет переходами между состояниями `NONE`, `GRABBING`, `HANGING` и `CLIMBING`.

### 2. Кинематографическая камера
- **Multi-axis Feedback**: Камера поддерживает офсеты по Pitch (наклон вниз при усилии), Roll (крен в сторону ведущей руки) и FOV (импульс скорости при выходе на блок).
- **Procedural Jitter**: Высокочастотные микро-вибрации имитируют мышечное напряжение в фазе максимального усилия подъема.
- **Asymmetric Animations**: Рандомизация ведущей руки (`climbSide`) инвертирует анимации крена, предотвращая визуальную монотонность.

### 3. Data-Driven Physics
- Все физические константы (длительность подъема, гравитация, сила прыжка) вынесены в `physics.json`.
- Это позволяет настраивать "ощущение" игры без перекомпиляции ядра.
