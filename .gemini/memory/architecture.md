...
### Mining & Interaction System (v2.2 UPDATED)
1. **Mining Controller**: Изолированный контроллер добычи. Управляет таймерами, Weak Spots и прогрессом. Теперь полностью очищен от логики выпадения предметов.
2. **Universal Loot System**: Логика выпадения предметов инкапсулирована в `BlockDefinition.spawnDrops`. Она учитывает как базовые дропы (`dropItem`, `dropChance`), так и сложные правила `DropRule` (инструменты, шансы). Предметы вылетают при любом вызове `World.destroyBlock`, что делает систему устойчивой к автоматизации.
3. **Block Survival (Neighbor Updates)**: 
    - Метод `World.setBlock` вызывает `notifyNeighbors`, который уведомляет 6 соседних блоков об изменении.
    - Блоки с флагом `requiresSupport` (например, трава) проверяют наличие опоры в `onNeighborChange` и саморазрушаются при её потере.
    - Реализована поддержка цепных реакций для `DOUBLE_PLANT`: разрушение одной части мгновенно вызывает `destroyBlock` для другой.
