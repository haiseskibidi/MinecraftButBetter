...
### Взаимодействие с миром и Оптимизация (v2.0 UPDATED)
1.  **Hit-based Block Breaking (Hytale-style v2.0)**:
    *   **Weak Spot Strategy**: Блоки поддерживают разные стратегии разрушения (`MiningSettings`). Для дерева/камня используется `weak_spots`: урон наносится эффективно только при попадании в процедурный маркер (белую зарубку).
    *   **Hit History Buffer**: Система хранит до 16 координат последних ударов. Эти данные передаются в шейдер как массив `uHitHistory[16]`, позволяя отрисовывать визуальные "шрамы" на блоке в местах попаданий.
    *   **Discrete Hits**: Разрушение блока основано на дискретных ударах, а не на непрерывном времени. Удар наносится только если `hitCooldownTimer <= 0.0f` (базово 0.35с).
    *   **Damage Math**: Базовый урон умножается на `missMultiplier` (0.2), если игрок промахнулся мимо слабой точки. `maxHealth = hardness * 10.0f`.
    *   **Progress Retention**: `breakingProgress` и `hitHistory` сохраняются, пока игрок смотрит на тот же блок. Отпускание ЛКМ не сбрасывает состояние.
2.  **Impact Wobble Animation**:
    *   **Procedural Chipping**: Вместо текстур трещин используется модуль `breaking_patterns.glsl`. Он рисует пиксельные зарубки, основываясь на `uWeakSpotPos` и истории ударов.
    *   **Impact Focus**: Эффект сруба (зарубки) визуально усиливается в точке, куда непосредственно наведен прицел (`uBreakingHitPoint`).
    *   **Proxy Rendering**: При ударе рендерер создает временный "прокси-меш" блока. Оригинальный блок скрывается (`discard`) в `fragment.glsl`.
    *   **Dynamic Hole Mesh**: Генерируется `holeMesh` для закрытия X-Ray дыр при сжатии прокси-блока.
    *   **Carving Synchronization**: Рендерер динамических масок (`CarvingRenderer`) синхронизирован с системой Wobble. Маска пня использует тот же `uIsProxy` и ту же матрицу трансформации, что и основной блок.
    *   **Cinematic Rotation**: Шейдер `vertex.glsl` применяет матрицы вращения вокруг центра `vec3(0.0, 0.5, 0.0)`.
    *   **Data-Driven Animation**: Настроена на упругое сжатие (scale < 1.0) и затухающую тряску.
3.  **Event-Driven Interactions**: Логика взаимодействия с блоками вынесена из `InputManager` в метод `onUse` класса `BlockDefinition`. Это позволяет легко добавлять новые типы поведения через JSON (`type`).
4.  **Lazy Ticking**: Система `ITickable` поддерживает метод `shouldTick()`. `World` пропускает обновление объектов, которые находятся в режиме ожидания (например, генератор без топлива), что значительно снижает нагрузку на CPU при большом количестве механизмов.
5.  **Neighbor Notification**: При `setBlock` мир уведомляет соседние чанки (`setNeedsMeshUpdate`), что исправляет прозрачность граней на стыках.
