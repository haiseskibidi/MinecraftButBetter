...
### Взаимодействие с миром и Оптимизация (v2.1 UPDATED)
1.  **World Damage & Scavenging System**:
    *   **Damage Map**: `World.java` хранит карту `Map<BlockPos, BlockDamageInstance>`. Каждая запись содержит текущий урон и `hitHistory` (координаты ударов). Это делает повреждения логически персистентными.
    *   **Healing & Scar Fading**: В методе `update` мира блоки с `healing_speed > 0` восстанавливают целостность. Количество шрамов в истории динамически уменьшается пропорционально HP, создавая эффект "заживления".
    *   **Dismantling (Scavenging)**: Механика "Разбора" основана на правилах `drop_on_hit` в JSON блока. Предметы вылетают из блока при попадании по слабым точкам, если в руках подходящий инструмент (например, Crowbar).
    *   **Mining Synchronization**: `MiningController` при начале добычи синхронизирует локальный список ударов с историей из мира.
2.  **Impact Wobble Animation**:
    *   **Procedural Chipping**: Модуль `breaking_patterns.glsl` рисует пиксельные зарубки на основе `uWeakSpotPos` и истории ударов.
    *   **Proxy Rendering**: При активном ударе рендерер создает временный "прокси-меш" блока для анимации тряски и отображения шрамов.
