...
- **IInventory (Interface)**: Базовый контракт для любого объекта, способного хранить `ItemStack`. Реализован в `Inventory` (игрок), `RegistryInventory` (творческая панель) и будущих контейнерах.
- **Slot**: Логическая обертка над конкретным индексом в `IInventory`. Содержит правила валидации (`Predicate<ItemStack>`) и тип слота.
- **SlotGroup**: Набор слотов с общим поведением. Поддерживает динамическую активацию через `activeSupplier` (например, слоты подсумка доступны только при наличии `BagComponent` в слоте аксессуара).
