...
1.  **Hit-based Block Breaking (Hytale-style)**:
    ...
    *   **Damage Math**: `maxHealth = hardness * 10.0f`. `damage = toolMiningSpeed` (или `Items.HAND` скорость).
    *   **Progress Retention**: `breakingProgress` сохраняется, пока игрок смотрит на тот же блок. Отпускание ЛКМ не сбрасывает трещины. Сброс происходит только при смене цели `Raycast`.
    *   **Anti-Autoclicker**: Кулдаун удара един для зажатия ЛКМ и для быстрых кликов, что ограничивает максимальную скорость добычи скоростью атаки инструмента.
2.  **Impact Wobble Animation**:
    *   **Proxy Rendering**: При ударе рендерер создает временный "прокси-меш" блока (`breakingMesh`) в координатах цели. Оригинальный блок скрывается (`discard`) в `fragment.glsl`.
    *   **Dynamic Hole Mesh**: Для предотвращения X-Ray эффекта (прозрачных дыр в мире) при сжатии прокси-блока, генерируется `holeMesh` — сетка, состоящая из внутренних граней соседних блоков. Она рендерится поверх дыры, создавая иллюзию сплошной материи.
    *   **Data-Driven Tracks**: Параметры смещения (`uWobbleOffset`), масштаба (`uWobbleScale`) и тряски (`uWobbleShake`) вычисляются на основе `block_wobble.json`.
    *   **Cinematic Rotation**: Шейдер `vertex.glsl` деформирует прокси-меш относительно центра `vec3(0.0, 0.5, 0.0)` и применяет математические матрицы вращения (на основе времени и `uWobbleShake`), создавая эффект упругого, тяжелого удара.
    *   **Single-Shot**: Анимация настроена как `looping: false`, проигрывается один раз за каждый успешный удар.
