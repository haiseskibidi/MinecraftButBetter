...
*   **Хирургические правки (КРИТИЧЕСКИ ВАЖНО):** 
    - При обновлении СУЩЕСТВУЮЩИХ файлов используй ТОЛЬКО инструмент `replace`.
    - Инструмент `write_file` разрешено использовать **ТОЛЬКО ДЛЯ СОЗДАНИЯ НОВЫХ ФАЙЛОВ**.
    - ЗАПРЕЩЕНО полностью перезаписывать существующие файлы через `write_file`!
